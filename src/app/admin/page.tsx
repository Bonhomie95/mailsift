...
export default function AdminPage() {
  ...
  if (authed === null) {
    return <div className="p-10 text-center text-white/40">Loading…</div>;
  }
  ...
  if (!authed) {
    return (
      <main className="mx-auto flex min-h-screen max-w-sm flex-col justify-center px-5">
        ...
        <p className="mb-6 text-sm text-white/40">Enter the admin password to manage providers.</p>
        <form onSubmit={login} className="space-y-3">
          <input
            ...
            className="w-full rounded-lg border border-white/15 bg-ink-800 px-3 py-2.5 text-sm outline-none focus:border-brand-400"
          />
        ...
        <a href="/" className="mt-6 text-center text-xs text-white/40 hover:text-white/70">
          ← Back to sorter
        </a>
      </main>
    );
    ...
    <main className="mx-auto max-w-4xl px-5 py-10">
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/40">Manage mail-provider matching rules.</p>
        </div>
      ...
      <section className="mb-8 rounded-2xl border border-white/10 bg-ink-800/60 p-5">
        ...
        <form onSubmit={addProvider} className="grid gap-3 sm:grid-cols-2">
          <input
            ...
            className="rounded-lg border border-white/15 bg-ink-900 px-3 py-2 text-sm outline-none focus:border-brand-400"
          />
          <input
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              className="w-20 rounded-lg border border-white/15 bg-ink-900 px-3 py-2 text-sm outline-none focus:border-brand-400"
            />
            ...
            <input
              ...
              className="h-9 w-12 rounded border border-white/15 bg-ink-900"
            />
          </div>
          <button
      ...
      <section className="mt-8 rounded-2xl border border-white/10 bg-ink-800/60 p-5">
        <h2 className="mb-1 text-sm font-semibold uppercase tracking-wide text-white/60">API keys</h2>
        <p className="mb-4 text-xs text-white/40">
          Give a key to programmatic callers. They pass{" "}
          <code className="rounded bg-white/10 px-1">Authorization: Bearer &lt;key&gt;</code> to{" "}
          <code className="rounded bg-white/10 px-1">POST /api/sort</code> and skip the free rate limit.
        </p>

        {!dbConfigured && (
          <p className="mb-3 text-xs text-amber-300">Requires Supabase (and the <code>api_keys</code> table).</p>
        )}

        {newKey && (
          <div className="mb-4 rounded-lg border border-emerald-400/30 bg-emerald-400/10 p-3 text-sm">
            <p className="mb-1 text-emerald-300">Copy this key now — it won&rsquo;t be shown again:</p>
            <code className="block break-all rounded bg-black/30 p-2 text-xs text-white/90">{newKey}</code>
          </div>
        )}
        ...
        <div className="mb-4 flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-white/15 bg-ink-900 px-3 py-2 text-sm outline-none focus:border-brand-400"
          />
          <button
            onClick={createKey}
            disabled={!dbConfigured}
        ...
        <div className="space-y-2">
          {keys.length === 0 && <p className="text-xs text-white/30">No API keys yet.</p>}
          {keys.map((k) => (
            <div key={k.id} className="flex items-center gap-3 rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-white/90">{k.label}</span>
                  {!k.active && <span className="rounded bg-red-500/15 px-1.5 py-0.5 text-[10px] uppercase text-red-300">revoked</span>}
                </div>
                <div className="truncate text-xs text-white/40">
              ...
              {k.active && (
                <button onClick={() => revokeKey(k.id)} className="text-xs text-red-300/70 hover:text-red-300">
                  Revoke
                </button>
              )}
            </div>
